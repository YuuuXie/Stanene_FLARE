...
class MappedForceField:
    ...
    def __init__(self, GP, grid_params, struc_params, mean_only=False):
...
class Map2body:
    ...
    def __init__(self, grid_num, bounds, GP, bond_struc, bodies='2', 
        ...
        if not load_prefix:
            ...
            y_mean = np.zeros([grid_num])
            y_var = np.zeros([grid_num, len(GP.alpha)])
            self.build_map(y_mean, y_var)
            if mean_only:
                self.mean.model.__coeffs__ = np.load(load_prefix+'_2.npy')
...
#            env2.cross_bond_dists = np.array([[0]])
    ...
    def predict(self, atom_env, GP, mean_only):
        ...
        atom_var = np.zeros(3)
        if not mean_only and not self.mean_only:
            if self.bodies == '2':
...
class Map3body:
    ...
    def __init__(self, grid_num, bounds, GP, bond_struc, bodies='3', 
        ...
        if not load_grid:
            ...
            if mean_only:
                self.mean.model.__coeffs__ = np.load(load_grid+'_3.npy')
