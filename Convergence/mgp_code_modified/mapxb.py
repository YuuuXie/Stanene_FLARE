...
class MapXbody:
    ...
    def predict(self, atom_env, rank=None):
        f_spcs = np.zeros(3)
        vir_spcs = np.zeros(6)
        v_spcs = 0
        e_spcs = 0
        ...
        for i, spc in enumerate(spcs):
            ...
            try:
                f, vir, v, e = self.maps[map_ind].predict(lengths, xyzs, rank)
                f_spcs += f
                vir_spcs += vir
                v_spcs += v
                e_spcs += e
...
class SingleMapXbody:
    ...
    def build_map(self, GP):
        ...
        if self.var_map == "pca" and self.svd_rank == "auto":
            ...
            self.var = PCASplines(
                ...
                self.bounds[1],
                orders=self.grid_num,
                svd_rank=np.min((G, y_var.shape[-1])),
            )
    ...
    def predict(self, lengths, xyzs, rank):
        """
        ...
        if self.var_map == "simple":
            ...
            if (rank is None) or (self.bodies == 2):
                rank = self.var.svd_rank
            elif rank > self.var.svd_rank and self.bodies == 3:
                print("Warning: change rank from", rank, "to", self.var.svd_rank)
                rank = self.var.svd_rank
            v_0 = self.var(lengths) # (svd_rank, len(lengths))
            v_0 = np.sum(v_0[:rank, :], axis=1) # (rank, )
            #v_0 = np.sum(v_0, axis=1) # (rank, )
            v_0 = np.expand_dims(v_0, axis=1) # (rank, 1)
            v = self.var.V[:, :rank] @ v_0
            #v = self.var.V @ v_0
